...
LOWEST_LEVEL_CMD="NCCL_P2P_DISABLE=1 conda run -n megatron python $TORCHTITAN_WORKSPACE/run_megatron.py --model $MODEL --nnodes $NNODE --nproc-per-node $NGPU --master-addr $HEAD_PRIVATE_IP --master-port $MASTER_PORT --disable-background-mode $PARALLEL_ARGS $NSIGHT_FLAG ${TRAIN_ARGS_QUOTED}"
