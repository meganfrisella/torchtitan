...
LOWEST_LEVEL_CMD="conda run -n torchtitan env PYTHONPATH=$TORCHTITAN_PYTHONPATH NNODE=$NNODE NGPU=$NGPU LOG_RANK=$LOG_RANK MODULE=$MODULE CONFIG=$CONFIG NODE_RANK=<node_rank> MASTER_ADDR=$HEAD_PRIVATE_IP MASTER_PORT=$MASTER_PORT $TRAIN_SCRIPT ${TRAIN_ARGS_QUOTED}"
